...
It is convenient to define $M_0(s) = M(1, s) + M(2, s)$ as the original
amount of mass drawn to create the sample before selective species loss and
to define
$M(1, s) = f_s M_0(s)$.  Since
...
\label{eq:XG}
\end{equation}
Again, if $g_i(r, s) = 1$ for all $i$ and for $r = 1,2$,
this reduces to the well-known result
\begin{equation}
...
\begin{equation}
\mu_{ij}^{(CI)}(CAI) = 10^6 \left(\frac{B_i(CAI, CI) - B_j(CAI, CI)}{B_j(CAI, CI)}\right)
\label{eq:mu_cai}
\end{equation}
