...
\title{Supplementary Materials (SM2): NRLEE Nucleosynthesis\footnote{Supplementay material for the paper
{\it The stellar building blocks of the Solar System: New constraints from correlated isotopic compositions in meteorites}, GCA (2021), authors: K. R. Bermingham,
...
\author{Klaus Mezger}
\affiliation{Institut f\"ur Geologie, Baltzerstrasse 1+3 3012 Bern, Switzerland}
...
(NSE),
the weak reactions drive the $Y_e$ to $\sim 0.48$. In the equilibrium,
...
If $^{95}$Zr condensed in NRLEE dust, then that dust could carry large
enrichments in daughter $^{95}$Mo.  Differing amounts of NRLEE dust in
Solar System reservoirs might then contribute to the offset
of correlated anomalies in $^{94}$Mo and $^{95}$Mo
seen in planetary materials from different (NC and
CC) regions \cite{2019NatAs...3..736B}.
...
\item NRLEE-Nucleosynthesis: \url{https://github.com/mbradle/NRLEE-Nucleosynthesis}  This repository also includes the latex for the present paper and
instructions for generating it (see the {\it doc/} subdirectory).
\end{itemize}
